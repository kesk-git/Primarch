...
A watcher whose state directory is missing or unwritable can never hold `state/.watch.lock`, because no lock owner can be created there.
It exits nonzero naming that directory instead of waiting on a lock nothing can hold or presenting a stale pid as a live holder, so restoring the directory is the operator action rather than stopping a peer.
Ordinary contention is unchanged: a live holder is still reported as the running watcher, and the blocking acquisitions the watcher makes for its own state locks still wait that holder out.
...
It also pins the unusable-lock-root behavior in both directions: acquisition on a removed lock root returns instead of spinning while a live holder is still waited out, and a watcher parked inside its startup reclaim whose state directory then disappears or loses write permission exits naming that directory rather than a phantom holder.
