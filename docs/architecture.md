...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, absorbed stale panes that persist past `FM_STALE_ESCALATE_SECS` without a pipeline-owned run on a provably idle pane at the moment the escalation would fire, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
The wedge escalation re-reads the authoritative crew state at the moment it would fire, because the classification that absorbed the pane may be many minutes old and elapsed idle time alone is not evidence of a wedge.
A pane MEASURED idle whose pipeline-owned run is still active is absorbed there rather than escalated: no-mistakes owns the crew for the run's duration, and a quiet pane is that run's normal shape.
The idle has to be a measurement: a pane whose busy state could not be classified at all escalates instead, because "not provably busy" merges a measured idle with a pane nobody could read, and only the first is evidence.
That exemption is deliberately narrower than "provably working" and covers only a pipeline-owned run on a provably idle pane, never a busy pane, which must keep escalating so `FM_BUSY_TURN_MAX_SECS` retains the only bound that catches a hung foreground call.
The crew-state line alone cannot make that distinction - a busy pane whose branch has a live run reports the same run-step verdict as an idle one - so the guard also requires the watcher's own busy observation of that pane from the same poll.
A crew whose state cannot be read still escalates, reported as unconfirmed rather than as a measured wedge verdict.
Repeated wedge escalations on the same unchanged pane add an escalation count to the wake reason and, at `FM_WEDGE_DEMAND_INSPECT_COUNT`, a `demand-deep-inspection` marker whose instruction names what was actually observed: a busy pane past its turn bound is reported as such even when the run source also failed to answer, so an unmeasured state never overwrites a measured one in the diagnosis.
...
Away mode's own stale-persistence escalator in `bin/fm-supervise-daemon.sh` consults the same shared absorb predicate before it alarms, so a provably idle pane under a live pipeline-owned run is exempt there too rather than only while the always-on watcher is triaging.
...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review: it attributes a no-mistakes run to a crew's branch, then keeps that run-step authoritative even if the pane has closed.
Attribution decides liveness first and uses code identity only to disambiguate finished runs.
That asymmetry is deliberate and load-bearing: the pipeline runs each step in its own clone, a separate object store from the crew worktree's, so every mid-run fix commit advances the run head to a commit the worktree cannot resolve, and a head match is therefore impossible for the whole time the run is healthiest.
A run the pipeline is still holding is attributed on branch alone, from the `branch_sync` block of `axi status` or an active row of the coarse runs list, while a finished run must still match the worktree's code identity so a historical run on a reused or rewritten branch is never reported as current.
The script header owns the exact run-head ancestry rules, and `bin/fm-nm-run-lib.sh` owns the shared status vocabulary and the head-binding limit.

That read also distinguishes three outcomes a supervisor must never confuse, because they justify opposite actions: a healthy running lane, a crew with no live run, and a run source that could not be read at all.
The last reports the distinct `unreadable` state naming why, and deliberately does not fall through to the status log, because promoting an append-only event line to current state is worst exactly when the authoritative source is down.
...
In away mode, seen-status dedupe does not clear possible-wedge aging for nonterminal progress, so housekeeping still ages an unchanged idle pane to the configured bound and, subject to the shared absorb predicate above, escalates it there.
...
Before merging, it also scans the PR's commits for a `Co-Authored-By:` trailer naming a known AI coding agent and refuses unless `FM_PR_MERGE_ALLOW_AGENT_COAUTHOR=1` confirms a genuine human name collision, so an agent-inserted trailer (AGENTS.md section 1) can never reach a merged task silently; a failed or unavailable forge lookup only warns and still merges, so this never blocks a legitimate external PR.
