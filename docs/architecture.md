...
That exemption is deliberately narrower than "provably working" and covers only a pipeline-owned run, never a merely busy pane, which must keep escalating so `FM_BUSY_TURN_MAX_SECS` retains the only bound that catches a hung foreground call.
