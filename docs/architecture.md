...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, absorbed stale panes that persist past `FM_STALE_ESCALATE_SECS` without a pipeline-owned run on an idle pane at the moment the escalation would fire, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
An idle pane whose pipeline-owned run is still active is absorbed there rather than escalated: no-mistakes owns the crew for the run's duration, and an idle pane is that run's normal shape.
That exemption is deliberately narrower than "provably working" and covers only a pipeline-owned run on an idle pane, never a busy pane, which must keep escalating so `FM_BUSY_TURN_MAX_SECS` retains the only bound that catches a hung foreground call.
...
Away mode's own stale-persistence escalator in `bin/fm-supervise-daemon.sh` consults the same shared absorb predicate before it alarms, so an idle pane under a live pipeline-owned run is exempt there too rather than only while the always-on watcher is triaging.
