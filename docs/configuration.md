...
It emits `FLEET_SYNC:` for skipped refreshes that may matter, recovered self-heals, `STUCK:` alarms, and `registry-invalid:` reports for a synced clone whose `data/projects.md` line does not parse to a known mode.
...
Independently of that refresh, every session start lints the whole `data/projects.md` registry and reports a line whose `[<mode> +yolo]` annotation does not parse to a known mode as `PROJECT_REGISTRY: <name>: <warning> - line: "<raw line>" - expected: - <name> [<mode> +yolo] - <desc> (added <date>)`, so a typo is caught even for a project this home never cloned, instead of every mechanical consumer silently reading that project's registered posture as the fallback `no-mistakes off`.
