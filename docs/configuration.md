...
It emits `FLEET_SYNC:` for skipped refreshes that may matter, recovered self-heals, `STUCK:` alarms, and `registry-invalid:` reports for a synced clone whose `data/projects.md` entry line violates the registry grammar owned by [`bin/fm-project-mode.sh`](../bin/fm-project-mode.sh)'s header, which enumerates every case it reports: `unknown mode`, `unrecognized annotation token`, `duplicate annotation token`, `empty annotation`, and `malformed annotation` (an unterminated `[`, or a token outside the brackets).
...
Independently of that refresh, every session start lints the whole `data/projects.md` registry through `fm-project-mode.sh --lint`, validating each entry line end to end against that same grammar, and reports every violation - the same five `<reason>` strings listed above - as `PROJECT_REGISTRY: <name>: <reason> - line: "<raw line>" - expected: - <name> [<mode> +yolo] - <desc> (added <date>)`, so a typo is caught even for a project this home never cloned, or on a duplicate entry for a name an earlier line already registered, instead of every mechanical consumer silently reading a posture the captain never registered.
A fault that leaves the mode token readable keeps that mode and drops only the faulty part (yolo falls back to `off`); only an unreadable or absent mode falls back to the standing `no-mistakes off`, and the warning says which happened.
A healthy registry, an absent registry, and a cloned-but-unregistered project all stay silent in both this lint and the `FLEET_SYNC:` report, as does a bullet that is not a registry entry at all (the grammar is an allowlist, so prose in `projects.md` is never diagnosed); [`bootstrap-diagnostics`](../.agents/skills/bootstrap-diagnostics/SKILL.md) owns the response procedure for both lines.
...
FM_CREW_STATE_RUNS_LIMIT=200  # recent no-mistakes run rows scanned when axi status cannot be attributed to this crew's branch
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before an absorbed stale pane escalates, unless the re-read at that moment shows a pipeline-owned run on a provably idle pane; stale panes whose crew is not provably working surface immediately unless they declare the pause verb
FM_BUSY_TURN_MAX_SECS=3600         # maximum age of a busy pane's latest state/<id>.turn-ended marker, or its state/<id>.meta spawn record before any turn completes, before the same wedge escalation used for a non-busy stale takes over; a busy pane is never absorbed by a live run there, so this bound always fires; inspection-only, never an automatic interrupt or restart
