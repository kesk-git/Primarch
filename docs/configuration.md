...
It emits `FLEET_SYNC:` for skipped refreshes that may matter, recovered self-heals, `STUCK:` alarms, and `registry-invalid:` reports for a synced clone whose `data/projects.md` line is malformed (an unknown mode, or any token inside the `[...]` annotation other than the mode and `+yolo`).
...
Independently of that refresh, every session start lints the whole `data/projects.md` registry through `fm-project-mode.sh --lint` and reports each malformed line - an unknown mode, or any token inside the `[<mode> +yolo]` annotation that is neither the mode nor `+yolo` - as `PROJECT_REGISTRY: <name>: <reason> - line: "<raw line>" - expected: - <name> [<mode> +yolo] - <desc> (added <date>)`, so a typo is caught even for a project this home never cloned, or on a duplicate entry for a name an earlier line already registered, instead of every mechanical consumer silently reading that project's registered posture as the fallback `no-mistakes off`.
A healthy registry, an absent registry, and a cloned-but-unregistered project all stay silent in both this lint and the `FLEET_SYNC:` report; [`bootstrap-diagnostics`](../.agents/skills/bootstrap-diagnostics/SKILL.md) owns the response procedure for both lines.
