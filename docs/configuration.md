...
FM_STALE_ESCALATE_SECS=240         # idle seconds before an absorbed stale pane escalates, unless the re-read at that moment shows a pipeline-owned run on an idle pane; stale panes whose crew is not provably working surface immediately unless they declare the pause verb
