...
`.no-mistakes.yaml` pins the gate's lint command to `bin/fm-lint.sh`, matching the Linux CI lint job; [`docs/configuration.md`](docs/configuration.md#gate-defaults-no-mistakesyaml) owns the rest of the gate defaults, including where the gate stores test evidence.
