...
No tmux target syntax can name a window whose name contains a literal `.`, because tmux splits the window part on `.` as the pane separator, so `has-session` fails against a live `fm-v1.2-fix` both anchored and unanchored.
Task ids may contain `.`, so presence must be read from the session's real window inventory (`list-windows -t "=<session>"` filtered with `grep -Fqx`, the shape `fm_backend_tmux_agent_state` already used), with an anchored `has-session` only as the fallback for pane ids, window ids, and `window.pane` targets.
