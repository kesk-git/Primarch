...
- A crew whose state reads `unreadable` still appears Underway rather than being hidden, because a crew nobody could measure is precisely the one a reader must see, but its line is LABELLED as unmeasured and never described as progressing: `unreadable` means the run source did not answer, not that the crew is working.
