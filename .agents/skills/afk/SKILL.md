...
- `stale` with a terminal status or bare legacy captain-relevant line -> escalate.
  ...
  If the pane is still idle past `FM_STALE_ESCALATE_SECS` (default 240s), housekeeping escalates it as a possible wedge - unless the shared absorb predicate reports a live pipeline-owned run AND the pane was MEASURED idle, in which case the quiet pane is that run's normal shape, the marker is reset, and away mode raises no alarm the always-on watcher would not raise either.
  A pane whose busy state could not be classified is not a measured idle and still escalates, because "not provably busy" merges that case with a real idle verdict and only the latter is evidence.
