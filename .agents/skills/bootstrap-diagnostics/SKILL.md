...
- `FLEET_SYNC: <repo>: registry-invalid: <detail>` / `PROJECT_REGISTRY: <name>: <detail> - line: "<raw line>" - expected: <format>` - the project's `data/projects.md` registry line does not parse to a known `[<mode> +yolo]` annotation, so every mechanical consumer (`bin/fm-project-mode.sh`, fleet sync, seeding, spawn's advisory notice) is reading that project's captain-registered posture as its silent default (`no-mistakes off`) instead of what the line intends.
