...
Pi's built-in footer stays visible while Firstmate adds `Claude <n>% left · GPT <n>% left`, refreshed at session start and every five minutes from `quota-axi`; an unavailable provider or failed read displays `n/a`.
