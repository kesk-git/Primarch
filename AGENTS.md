...
.no-mistakes/        local validation state; gitignored (docs/configuration.md owns the gate's test-evidence posture)
```
...
Judge validation by the run step `bin/fm-crew-state.sh` attributes to that crew, not by shell liveness or the last status event; that attribution decides liveness first and uses code identity only to disambiguate a finished run.
